<script lang="ts">
  ...
  import { prefixesDefault } from "$lib/shared-defaults.js";
  ...
  let status: "closed" | "pasting" | "preview" | "generate" | "done" | "error" =
    $state("closed");
  ...
  let extractedMarkers = $derived.by(() => {
    return pastedState
      .split("\n")
      .flatMap((line) => {
        const matchingPrefix = Object.entries(prefixes).find(([_, prefix]) =>
  ...
  function retrier(maxAttempts: number) {
    let attempt = 0;
    return async function retry<T extends () => Promise<any>>(
      fn: T,
  ...
  async function createScreenshots(markers: MarkerDetails[]) {
    ...
    await eachLimit(
      markers,
      maxParallelRequests,
      async (marker: MarkerDetails, callback: () => void) => {
...
{#if status !== "closed"}
  <Modal title="Screenshot tool" {onClose} {footerChildren}>
    <div class="screenshot-tool">
      ...
      {#if status === "preview"}
        <p>
          This will create {extractedMarkers.length} screenshot{extractedMarkers.length ===
          1
            ? ""
            : "s"}:
        </p>
            ...
            {#each extractedMarkers as { name, data }, i}
              <tr
                ><td>{i + 1}</td><td>{name}</td><td
                  ><code class="marker">{data}</code></td
                ></tr
              >
            {/each}
          </tbody>
